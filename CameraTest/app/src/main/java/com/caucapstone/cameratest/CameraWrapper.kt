...
var _selectedFilterType = FilterType.FILTER_NONE
...
suspend fun Context.createImageAnalyzerUseCase(
    lifecycleOwner: LifecycleOwner,
    cameraSelector: CameraSelector,
    previewView: PreviewView,
    filterImageView: ImageView,
    colorCodes: MutableState<Triple<Int, Int, Int>>,
    colorName: MutableState<String>,
    selectedFilterType: FilterType
) {

    _selectedFilterType = selectedFilterType
    _selectedFilterType = FilterType.FILTER_SPECIFIC
...
//            }
        ...
        if(_selectedFilterType == FilterType.FILTER_SPECIFIC) {
            callBackBitMap(coverFilter(rgbBitMap, hueCriteria))
        }
        //Dalton색상으로 변경하기
        else if(_selectedFilterType == FilterType.FILTER_DALTONIZED){
            callBackBitMap(daltonFilter(rgbBitMap,blindType))
        }
        //잘 안보이는 색상에 빗금 치기
        else if(_selectedFilterType == FilterType.FILTER_STRIPE){
